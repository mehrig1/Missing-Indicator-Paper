...
#--------------------------------------------------------------#
#### MAR v2 (indicators not in outcome simulation) 20% missing ####
#--------------------------------------------------------------#
...
for (i in 1:n_sims) {
  ...
  # move forward with df_MAR
  df_MAR <- df_long
  
  # linear predictor for probability of being missing
  df_MAR$LP <- (-2.8 + 0.2 * scale(df_MAR$ui) + 0.2 * scale(df_MAR$diab) +
  ...
  # used to calculate overall percent missing
  ...
  # for binary missing variables, want everything as 1 except the variable 
  # itself and cluster variable (-2); also indicator for variable set to 0
...
write.csv(m_results, "MAR v2 20.csv")