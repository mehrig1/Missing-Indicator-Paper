#--------------------------------------------------------------#
#### MNAR v2 (indicators not in outcome simulation) 20% missing ####
#--------------------------------------------------------------#
...
for (i in 1:n_sims) {
  ...
  set.seed(124 + i) # set seed, i begins at 1
  ...
  # setting bounds - decision to cap only lower values
  ...
  df_long$e_ij_slb <- rnorm(n = n_tobs, mean = slb_ran_error_mean, sd = slb_ran_error_sd) # random error
  ...
  # df_long$SLB <- ifelse(df_long$SLB > 30, 30, df_long$SLB)
  ...
  # rounding age BMI, GS, and BMI to 2 decimal places
  ...
  # creating df_MNAR data set, proceeding with df_MNAR
  df_MNAR <- df_long
  
  # defining probability of being missing for each variable
  df_MNAR$prob_missing_GS <- ifelse(df_MNAR$SLB > quantile(df_MNAR$SLB, 0.25), 0, 0.3)
  df_MNAR$prob_missing_SLB <- ifelse(df_MNAR$GS > quantile(df_MNAR$GS, 0.25), 0, 0.3)
  df_MNAR$prob_missing_pm <- ifelse(df_MNAR$pain_meds == 0, 0, 0.1)
  df_MNAR$prob_missing_dm <- ifelse(df_MNAR$dep_meds == 0, 0, 0.1)
  ...
  # used to get overall percent of missingness
  ...
  df_MNAR_ind <- df_MNAR_mis %>% select(
    -u_0i_falling, -prob_falling, -prob_missing_GS, -prob_missing_SLB,
    -prob_missing_pm, -prob_missing_dm,
  ...
  # and cluster variable (-2)
  # indicator for specific variable will also be set to 0 in matrix
  ...
  # capping lower values for GS and SLB
...
write.csv(m_results, "MNAR v2 20.csv")
